...
describe('C8-4 hardened Chapter 8 flashcards', () => {
  it('preserves all original 50 IDs and the targeted 57-card expansion', () => {
    ...
    expect(ids.slice(50)).toEqual(
      Array.from({ length: 7 }, (_, index) =>
        `fc-8-${String(index + 51).padStart(3, '0')}`,
      ),
    )
  ...
  it('preserves exactly one primary concept mapping per current flashcard', () => {
    ...
    expect(new Set(chapter8FlashcardConceptMappings.map((mapping) => mapping.flashcardId)).size).toBe(57)
  })
  ...
  it('documents the remaining thin concept families before expansion', () => {
    ...
    expect(counts['ch8-microcurrent-high-frequency']).toBe(5)
    expect(counts['ch8-light-therapy-safety']).toBe(5)
  })
})
