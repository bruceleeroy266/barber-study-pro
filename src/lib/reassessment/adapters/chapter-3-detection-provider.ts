/**
 * C3-3 Stage 3 — Chapter 3 Concept Detection Provider
 *
 * Concrete implementation of IConceptDetectionProvider for Chapter 3.
 * Bridges the locked Chapter 3 concept runtime to the chapter-agnostic
 * EvaluationService.evaluateCycleWithDetection() — the same contract
 * Chapter 2's detection provider satisfies.
 *
 * Architecture:
 *   - Reuses the canonical question→concept mappings (initial + reserve)
 *     from chapter-3-concepts/mappings.ts
 *   - Delegates to the shared detection engine through the Chapter 3 binding
 *     (buildConceptEvidence + detectConceptState from chapter-3-concepts/detection)
...
import {
  ...
  detectConceptState,
  type ConceptDetectionResult as Chapter3BindingResult,
} from '@/lib/chapter-3-concepts/detection'
import type { Chapter3ConceptFamilyId } from '@/lib/chapter-3-concepts/types'
...
/**
 * Configuration for Chapter3DetectionProvider.
 */
export interface Chapter3DetectionProviderConfig {
  /**
...
// ───────────────────────────────────────────────
// Chapter 3 Detection Provider
// ───────────────────────────────────────────────

/**
 * Chapter 3 implementation of IConceptDetectionProvider.
 *
 * This provider:
 *   1. Validates that the concept exists in the locked Chapter 3 taxonomy
 ...
 *   4. Delegates to the shared engine (via the Ch3 binding) for evidence + state
 */
export class Chapter3DetectionProvider implements IConceptDetectionProvider {
  ...
  constructor(config: Chapter3DetectionProviderConfig) {
    this.fetchQuizAttempts = config.fetchQuizAttempts

    // Build canonical mapping lookup from chapter-3-concepts/mappings.ts
    // (initial 30 + reassessment reserve 60 — projected, never restated)
    this.conceptToQuestionsMap = new Map()
    this.validConceptIds = new Set(CHAPTER3_CONCEPT_FAMILY_IDS as readonly string[])
  ...
  async detectConceptState(
    ...
    // Validate concept exists in the locked Chapter 3 taxonomy
    ...
    if (!hasMappedQuestions) {
      ...
      const emptyEvidence = buildConceptEvidence(conceptId as Chapter3ConceptFamilyId, [])
      const result = detectConceptState(emptyEvidence)
      return this.mapToProviderResult(result)
    }

    // Build concept evidence using the shared engine (Ch3 binding)
    const evidence = buildConceptEvidence(
      conceptId as Chapter3ConceptFamilyId,
      filteredAttempts
    )

    // Detect state using the shared engine (Ch3 binding)
  ...
  /**
   * Map the Ch3 binding's detection result to the provider result format.
   */
  private mapToProviderResult(result: Chapter3BindingResult): ConceptDetectionResult {
    return {
...
/**
 * Create a Chapter 3 detection provider with the given configuration.
 */
export function createChapter3DetectionProvider(
  config: Chapter3DetectionProviderConfig
): Chapter3DetectionProvider {
  return new Chapter3DetectionProvider(config)
}
