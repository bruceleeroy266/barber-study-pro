  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--color-brand-gold)] hover:bg-[var(--color-brand-gold-light)] text-black font-medium rounded-lg transition-colors"
      >
      ...
      >
        <section id="student-report" className="report-section bg-white text-black rounded-xl p-2 print:shadow-none">
          ...
          <div className="flex items-center justify-between mb-6 border-b border-gray-200 pb-4">
            <div>
              <h2 className="text-2xl font-bold text-white">Student Progress Report</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="border border-gray-200 rounded-lg p-4">
              <div className="text-2xl font-bold text-white">{readiness.label}</div>
              <div className="text-xs text-silver-gray">Board Readiness ({readiness.score})</div>
            </div>
            <div className="border border-gray-200 rounded-lg p-4">
              <div className="text-2xl font-bold text-white">{boardRisk.label}</div>
              <div className="text-xs text-silver-gray">Board Exam Risk</div>
            </div>
          </div>

          {/* Chapter Progress */}
          <div className="mb-6 print-break-inside">
            <h3 className="text-lg font-bold text-white mb-3">Chapter Progress</h3>
            {chapters.length > 0 ? (
              ...
              <p className="text-silver-gray">No chapter data available.</p>
            )}
          </div>

          {/* Recent Quiz Attempts */}
          <div className="mb-6 print-break-inside">
            <h3 className="text-lg font-bold text-white mb-3">Recent Quiz Attempts</h3>
            {attemptRecords.length > 0 ? (
              ...
              <p className="text-silver-gray">No quiz attempts yet.</p>
            )}
          </div>

          {/* Weak Areas */}
          <div className="mb-6 print-break-inside">
            <h3 className="text-lg font-bold text-white mb-3">Weak Areas & Study Focus</h3>
            {!hasEnoughQuizData ? (
              <p className="text-silver-gray">Not enough quiz data yet.</p>
            ) : weakAreas.length > 0 ? (
              ...
              <p className="text-silver-gray">No weak areas found.</p>
            )}
          </div>

          {/* Instructor Notes */}
          <div className="mb-6 print-break-inside">
            <h3 className="text-lg font-bold text-white mb-3">Instructor Notes</h3>
            {noteRecords.length > 0 ? (
              <div className="space-y-3">
                {noteRecords.map((note) => (
                  <div key={note.id} className="border border-gray-200 rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-semibold uppercase bg-gray-100 text-silver-gray px-2 py-0.5 rounded">
                        {note.note_type}
                      </span>
                      <span className="text-xs text-silver-gray">by {note.instructor_name} • {formatDate(note.created_at)}</span>
                    </div>
                    <p className="text-sm text-light-gray whitespace-pre-wrap">{note.note_text}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-silver-gray">No instructor notes yet.</p>
            )}
          </div>
