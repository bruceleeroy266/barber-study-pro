  ...
  return (
    <div className="bg-charcoal border border-graphite rounded-xl p-6">
      ...
      <div className="flex flex-col items-center mb-6">
        <div className="w-32 h-32 relative">
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
            ...
            />
            <path
              className={readiness.isReady ? 'text-gold' : 'text-warm-bronze'}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={`text-2xl font-bold ${readiness.isReady ? 'text-gold' : 'text-warm-bronze'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-black border border-graphite rounded-lg p-3">
      ...
      {!readiness.isReady && readiness.remainingItems.length > 0 && (
        <div className="mt-4">
          <p className="text-sm font-medium text-light-gray mb-2">Remaining Items</p>
          <ul className="space-y-1.5">
            {readiness.remainingItems.map((item, idx) => (
              <li key={idx} className="text-sm text-silver flex items-start gap-2">
                <span className="text-warm-bronze mt-0.5">•</span>
                {item}
              </li>
            ))}
          </ul>
