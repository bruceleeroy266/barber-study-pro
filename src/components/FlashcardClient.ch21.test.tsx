/**
 * ASCYN PRO — chapter 21 FlashcardClient Tests
 *
 * Validates the chapter 21 deck loads, navigates, tracks progress,
...
function renderFlashcards(userId: string | undefined = 'user-1') {
  return render(
    <FlashcardClient
      flashcards={chapter21PremiumFlashcards}
      chapterId="ch-21"
      userId={userId}
      isCompleted={false}
    />
...
describe('FlashcardClient — chapter 21', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('flags a card for additional practice', async () => {
    ...
    expect(mocks.insert).toHaveBeenCalledWith({
      user_id: 'user-1',
      chapter_id: 'ch-21',
      flashcard_id: chapter21PremiumFlashcards[0].id,
    })
  })
  ...
  it('flips a card to reveal the back content', async () => {
    ...
    const card = screen.getByText(chapter21PremiumFlashcards[0].front)
    fireEvent.click(card)

    await waitFor(() => {
      expect(screen.getByText(chapter21PremiumFlashcards[0].back)).toBeInTheDocument()
    })
  })
  ...
  it('marks study complete after reaching the last card', async () => {
    ...
    const nextButton = screen.getByRole('button', { name: /Next →/i })
    for (let i = 0; i < chapter21PremiumFlashcards.length - 1; i++) {
      fireEvent.click(nextButton)
    }
  ...
  it('preserves progress in localStorage when userId is not available', async () => {
    ...
    expect(localStorage.getItem('flashcard-index-ch-21')).toBe('1')
  })
})
