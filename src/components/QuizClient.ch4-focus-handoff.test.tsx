/**
 * QuizClient Chapter 4 Focused-Review Handoff (C4-2)
 *
 * Component coverage for the Chapter 4 detection handoff through the generic
 * pipeline:
 ...
 *   - the handoff POST carries chapterId 'ch-4' + the exact persisted attempt
...
const mocks = vi.hoisted(() => {
  ...
  const single = vi.fn().mockResolvedValue({ data: { id: 'attempt-ch4-1' }, error: null })
...
const question: QuizQuestion = {
  id: 'qq-4-001',
  quiz_id: 'quiz-4',
  question: 'Which group contains the major types of pathogens a barber should recognize?',
  ...
  correct_answer: 'a',
  explanation: 'Bacteria, viruses, fungi, and parasites are major pathogen groups.',
  difficulty: 'easy',
  order_index: 1,
}
...
function makeFiveQuestions(): QuizQuestion[] {
  return Array.from({ length: 5 }, (_, index) => ({
    ...question,
    id: `qq-4-pass-${index + 1}`,
    question: `Chapter 4 concept check ${index + 1}?`,
    order_index: index + 1,
  }))
}

describe('QuizClient Chapter 4 focused-review handoff', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.restoreAllMocks()
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ...
        json: vi.fn().mockResolvedValue({
          ...
          cycleIds: ['cycle-ch4-focus-1'],
          conceptsDetected: ['ch4-pathogens-transmission'],
        }),
      })
    )
  })

  it('offers the newly created focus area directly from a failed Chapter 4 result', async () => {
    render(
      <QuizClient
        quiz={quiz}
        questions={[question]}
    ...
    expect(focusLink).toHaveAttribute('href', '/dashboard/remediation/cycle-ch4-focus-1')
    expect(screen.getByText(/A focused review is ready for you/i)).toBeInTheDocument()
  })

  it('keeps a detected concept gap visible even when the overall Chapter 4 score passes', async () => {
    ...
    expect(focusLink).toHaveAttribute('href', '/dashboard/remediation/cycle-ch4-focus-1')
    expect(screen.getByRole('link', { name: /Continue to Chapter 5/i })).toHaveAttribute(
      'href',
      '/dashboard/chapters/5'
    )
  })

  it('posts the Chapter 4 chapter ID and exact persisted attempt ID to the detection endpoint', async () => {
    render(
      <QuizClient
        quiz={quiz}
        questions={[question]}
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({ chapterId: 'ch-4', quizAttemptId: 'attempt-ch4-1' }),
        })
      )
    })
  })
