...
import type {
  Chapter8MicroCheck,
  Chapter8MicroCheckAnswer,
} from '@/lib/chapter-8-concepts/micro-checks'
import type { Chapter8MicroCheckAttemptRow } from '@/lib/chapter-8-concepts/micro-check-persistence'
import { persistChapter8MicroCheckAttempt } from '@/lib/chapter-8-concepts/micro-check-persistence'

interface Props {
  check: Chapter8MicroCheck
  userId: string
  theme: ChapterTheme
  attempts: readonly Chapter8MicroCheckAttemptRow[]
  onAttemptPersisted: (row: Chapter8MicroCheckAttemptRow) => void
}

export default function Chapter8MicroCheckCard({
  check,
  userId,
  theme,
  attempts,
  ...
  const [selected, setSelected] = useState<Record<string, Chapter8MicroCheckAnswer | undefined>>({})
  ...
  const submit = async (questionId: string) => {
    ...
    setError(null)
    const result = await persistChapter8MicroCheckAttempt(userId, check, question, answer)
    setSaving(null)
