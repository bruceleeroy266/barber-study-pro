...
begin
  ...
  select school_id into v_school from public.profiles where id = v_user;
  ...
  if v_record.id is null then
    ...
    values
      (v_school, v_user, current_date, 'Clocked In', now(), null, null, null)
...
begin
  ...
  if v_record.id is null or v_record.clocked_in_at is null then
    raise exception 'No active clock-in found';
  end if;
  if v_record.clocked_out_at is not null then
